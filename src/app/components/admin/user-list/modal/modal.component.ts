...
export class ModalComponent implements OnInit {
  ...
  save() {
    console.log(this.selectedSuperCategory,'s')
    ...
    if (
      ...
      this.$http.bulkDelete("http://localhost:3000/api/v1/user-area", this.selectedUser.UserAreas);
    }
    ...
    if (this.selectedAreas && this.selectedAreas.length) {
      const areas = this.selectedAreas.map(el => ({ userId: this.selectedUser.id, areaId: el }));
      areas.forEach(area=> {
        this.$http.rawPost("http://localhost:3000/api/v1/user-area", area)
        .subscribe(res=> {console.log(res, 'selectedAreas')})
      })
    }
